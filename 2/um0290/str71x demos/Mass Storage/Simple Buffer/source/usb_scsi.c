...
void SCSI_ReadFormatCapacity_Cmd(void)
{
  ...
  ReadFormatCapacity_Data[11] = (u8)(MASS_MEMORY_SIZE);
...
void SCSI_ReadCapacity10_Cmd(void)
{
  ...
  ReadCapacity10_Data[7] = (u8)(MASS_MEMORY_SIZE);
...
void SCSI_Read10_Cmd(void)
{
  ...
  {
    if (!(Address_Management_Test(SCSI_READ10)))// adress out of range
    {
      return;
    }
...
void SCSI_Write10_Cmd(void)
{
  ...
  if (Bot_State == BOT_IDLE)
  {
    if (!(Address_Management_Test(SCSI_WRITE10)))// adress out of range
    {
      return;
    }
