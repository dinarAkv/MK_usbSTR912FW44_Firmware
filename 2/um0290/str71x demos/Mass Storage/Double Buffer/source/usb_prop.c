...
{
  /* Set Mass Storage as not configured */
  pInformation->Current_Configuration = 0;
...
RESULT MASS_NoData_Setup(u8 RequestNo)
{
      ...
      && (pInformation->USBwIndex == 0) && (pInformation->USBwLength == 0x00))
  {
    ClearDTOG_RX(ENDP2);
    ClearDTOG_TX(ENDP2);
    ToggleDTOG_TX(ENDP2); /* reset value of the data toggle bits for the µ
                             endpoint out*/
    ClearDTOG_RX(ENDP1);
    ClearDTOG_TX(ENDP1); /* clear the data toggle bits for the endpoint IN*/
    CBW.dSignature = BOT_CBW_SIGNATURE; /*intialise the CBW signature to enable 
                                         the clear feature*/
    Bot_State = BOT_IDLE; /* set the Bot state machine to the IDLE state */
    return USB_SUCCESS;
  }
