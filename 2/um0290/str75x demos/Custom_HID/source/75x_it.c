...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
EXTIT_InitTypeDef   EXTIT_InitStructure1;  
...
{
  ADC_ConvertedValueX = ADC_GetConversionValue(ADC_CHANNEL3);
  
  Send_Buffer[0] = 0x07;
  
  if((ADC_ConvertedValueX >>2) - (ADC_ConvertedValueX_1 >>2) > 2)
  {
...
{
  EXTIT_InitStructure1.EXTIT_ITLine = EXTIT_ITLine7;
  EXTIT_InitStructure1.EXTIT_ITLineCmd = ENABLE;
  
  if(EXTIT_GetITStatus(EXTIT_ITLine7) != RESET)
  {
    Send_Buffer[0] = 0x05;
    
    if (GPIO_ReadBit(GPIO1,GPIO_Pin_5) == 1)
    {  
      Send_Buffer[1] = 0x00;

      /* switch Line 7 edge */
      EXTIT_InitStructure1.EXTIT_ITTrigger = EXTIT_ITTrigger_Falling;
      EXTIT_Init(&EXTIT_InitStructure1);
    }
    else if (GPIO_ReadBit(GPIO1,GPIO_Pin_5) == 0)
    {
      Send_Buffer[1] = 0x01;

      /* switch Line 7 edge */
      EXTIT_InitStructure1.EXTIT_ITTrigger = EXTIT_ITTrigger_Rising;
      EXTIT_Init(&EXTIT_InitStructure1);
    }    
  }
  ...
  /* Clear the EXTIT line 7 pending bit */
