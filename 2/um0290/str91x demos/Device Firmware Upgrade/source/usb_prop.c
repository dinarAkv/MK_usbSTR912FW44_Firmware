...
#define Bank0_Gap        0x80000
#define Flash_Max_Size   0x90000
#endif 

#ifdef Flash_2MB_1MB
#define Bank0_Gap        0x200000
#define Flash_Max_Size   0x220000
#endif 
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
    ...
    if (DeviceState == STATE_dfuDNBUSY)
    {
      if (wBlockNum == 0)   /* Decode the Special Command*/
      {
        ...
        else if (( Load_Buffer[0] ==  CMD_ERASE ) && (wlength == 5))
        {
          ...
          if  (Pointer < Bank0_Gap)  /* Internal Flash */
          {
            Internal_FLASH_SectorErase(Pointer + Bank0_Gap);
          }
        }
      }
      else if (wBlockNum > 1)  // Download Command
      {
        ...
        for (i = 0; i < wlength; i = i + 4)
        {
          ...
          if (Addr < Bank0_Gap)   /* Internal flash programming */
          {
            Internal_FLASH_WordWrite(Addr + Bank0_Gap, DATA);
          }
          Addr += 4;
        }
      }
...
u8 *UPLOAD(u16 Length)
{
  ...
  else if (wBlockNum > 1)
  {
    ...
    if (Pointer < Bank0_Gap)
    {
      Phy_Addr = (u8*)(Addr + Bank0_Gap);
    }
    if ((Pointer >= Bank0_Gap) && (Pointer < Flash_Max_Size)) /* Read bank1 */
    {
      Phy_Addr = (u8*)(Addr - Bank0_Gap);
    }
  ...
  switch (DeviceState)
  {
    case   STATE_dfuDNLOAD_SYNC  :
      ...
      if (wlength != 0)
      {
        ...
        {
          if  (Pointer <= Bank0_Gap) /* 64K sectors */
          {
            DeviceStatus[1] = 0x08;  /* 1.8 seconds */
            DeviceStatus[2] = 0x07;
            DeviceStatus[3] = 0x00;
          }
        }
        ...
        {
          DeviceStatus[1] = 80; /* 8ms for 1024 bytes */
          DeviceStatus[2] = 0;
          DeviceStatus[3] = 0;
        }
        break;
