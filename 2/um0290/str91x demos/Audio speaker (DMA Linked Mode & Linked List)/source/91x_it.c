...
/* Private variables ---------------------------------------------------------*/
u16 Pulse = 0;
u32 Stream_Buffer_Selector = 0;
u8* Stream_Buffer_Pointer;
u32 Out_Data_Offset;

/* Extern variables ----------------------------------------------------------*/
extern u8 Stream_Buff0[96];
extern u8 Stream_Buff1[96];
...
{
  Pulse += TIMER_PULSE_LENGTH_24KHZ;
  if ((Out_Data_Offset < 96) && ((u8)(MUTE_DATA) == 0))
  {
    /* Play the the Audio Stream */
    TIM_SetPulse(TIM3, TIM_PWM_OC1_Channel, *(Stream_Buffer_Pointer + Out_Data_Offset));
    Out_Data_Offset++;
  }
...
void DMA_IRQHandler(void)
{
 if (Stream_Buffer_Selector)
  {
    Stream_Buffer_Selector = 0;
    Stream_Buffer_Pointer = &Stream_Buff1[0];
  }
  else
  {
    Stream_Buffer_Selector = 1;
    Stream_Buffer_Pointer = &Stream_Buff0[0];
  }

  Out_Data_Offset = 0;
  Set_DMA_List();
  DMA_ClearIT(Channel0, DMA_TCC);          
