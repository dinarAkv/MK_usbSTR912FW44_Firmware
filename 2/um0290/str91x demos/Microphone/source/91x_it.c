...
/* Private variables ---------------------------------------------------------*/
u16 Pulse = 0;
u8 SampleN_1Value = 0x40;
/* Extern variables ----------------------------------------------------------*/
extern u32 Micro_Start;
...
vu8 SampleNValue;
  ...
  if (Micro_Start && (Data_Index < 22))
  {
    /* Get the ADC conversion value for sample N */
    /* A numeric RIF filter is applayed to the converted value */
    SampleNValue = 0.68 * ((((ADC_GetConversionValue(ADC_Channel_7)) - 0x150)) + 0x20) + (1 - 0.68) * SampleN_1Value;
    /* Store the converted sample N value */
    Stream_Buff[Data_Index++] = SampleNValue;
    /* Set actuel N sample value as N-1 sample for next record */
    SampleN_1Value = SampleNValue;
  }
  /* Set the TIM1 OC1 value */
  TIM_SetPulse(TIM1, TIM_PWM_OC1_Channel, Pulse);
